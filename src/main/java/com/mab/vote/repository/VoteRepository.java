...
public interface VoteRepository extends JpaRepository<VoteViewVO, Object> {
	...
	@Query(nativeQuery = true, value 
			= "select * from voteview where vote_no = ?1 order by content_no")
	public List<VoteViewVO> select_one_vote(String vote_no);
	
	@Query(nativeQuery = true, value = "SELECT CONTENT_NO, COUNT(*) AS CNT FROM ( "
			+ "SELECT * FROM VOTERESULT ORDER BY CONTENT_NO" + ") " + "WHERE VOTE_NO = ?1 GROUP BY CONTENT_NO")
	public List<Object> select_one_vote_result(String vote_no);

	@Query(nativeQuery = true, value = "SELECT content_no FROM VOTERESULT WHERE vote_no = ?1 AND user_no = ?2")
	public String select_one_my_vote_result(String vote_no, String user_no);

	@Transactional
	@Modifying
	@Query(nativeQuery = true, value = "INSERT INTO VOTE(VOTE_NO, VOTE_TITLE, VOTE_INFO, VOTE_EOD, VOTE_STATE, USER_NO, MEET_NO, ACTIVITY_NO, PRIVATE_STATE) "
	...
	@Transactional
	@Modifying
	@Query(nativeQuery = true, value = "INSERT INTO VOTECONTENT(CONTENT_NO, VOTE_NO, VOTE_CONTENT) "
	...
	@Transactional
	@Modifying
	@Query(nativeQuery = true, value = "UPDATE VOTE SET VOTE_STATE = 'N' WHERE VOTE_NO = :#{#vo2?.vote_no}")
	public int SQL_UPDATE_VOTE_STATE(@Param("vo2") VoteVO vo2);

	@Transactional
	@Modifying
	@Query(nativeQuery = true, value = "DELETE FROM VOTE WHERE VOTE_NO = :#{#vo2?.vote_no}")
	public int SQL_DELETE_VOTE(@Param("vo2") VoteVO vo2);

	@Transactional
	@Modifying
	@Query(nativeQuery = true, value = "INSERT INTO VOTERESULT(VOTE_RESULT_NO, VOTE_NO, USER_NO, CONTENT_NO) "
	...
	@Transactional
	@Modifying
	@Query(nativeQuery = true, value = "UPDATE VOTERESULT SET CONTENT_NO = :#{#vo2?.content_no} "
