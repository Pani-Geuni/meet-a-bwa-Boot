...
public interface ActivityCreateUpdateRepository extends JpaRepository<ActivityVO, Object> {
	...
	@Modifying
	@Transactional
	@Query(nativeQuery = true, value="update activity set activity_name=:#{#avo?.activity_name}, activity_info=:#{#avo?.activity_info}, activity_city=:#{#avo?.activity_city}, activity_county=:#{#avo?.activity_county}, activity_interest=:#{#avo?.activity_interest}, activity_gender=:#{#avo?.activity_gender}, activity_nop=:#{#avo?.activity_nop}, activity_age=:#{#avo?.activity_age} where activity_no=:#{#avo?.activity_no} and current_date >= :#{#avo?.activity_etime}")
